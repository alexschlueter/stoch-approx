...
\begin{lem}[Substitution bei Unabhängigkeit]
 Für $n\geq 1$ und jede Funktion $f:(I\times \zz,\bb(I)\otimes\cc)\to(\RR,\bb(\RR))$ mit
...
\begin{lem}\label{lem:misch}
  ...
  für $n\to\infty$. Außerdem sei $(Z_n)_{n\geq 1}$ eine Folge von reellen
  Zufallsvariablen mit $Z_n\to 0$ \textit{in Wahrscheinlichkeit}. Dann gilt
\begin{equation*}
...
\begin{align*}
  \expect{f\cdot h(Y_n+Z_n)}=\expect{f\cdot h'(Y_n,Z_n}&\to\int f\dvar{P}\int h'(y,z) \dvar{\nu\otimes\delta_0(y,z)}\\
